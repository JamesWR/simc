...
//#if __cplusplus >= 201103L
  ...
  js::sc_js_t to_json(const sim_t& sim)
  {
    ...
    static_assert( !std::is_void<decltype( to_json( std::declval<timespan_t>() ) )>::value == true, "" );

    node.set("debug", sim.debug );
    node.set("max_time", sim.max_time );
    /*
    node.set("expected_iteration_time", sim.expected_iteration_time );
    ...
    node.set("queue_lag", sim.queue_lag );
    node.set("queue_lag_stddev", sim.queue_lag_stddev );
    node.set("gcd_lag", sim.gcd_lag );
    node.set("gcd_lag_stddev", sim.gcd_lag_stddev );
    node.set("channel_lag", sim.channel_lag );
    node.set("channel_lag_stddev", sim.channel_lag_stddev );
    node.set("queue_gcd_reduction", sim.queue_gcd_reduction );
    ...
    node.set("world_lag", sim.world_lag );
    node.set("world_lag_stddev", sim.world_lag_stddev );
    node.set("travel_variance", sim.travel_variance );
    node.set("default_skill", sim.default_skill );
    node.set("reaction_time", sim.reaction_time );
    node.set("regen_periodicity", sim.regen_periodicity );
    node.set("ignite_sampling_delta", sim.ignite_sampling_delta );
